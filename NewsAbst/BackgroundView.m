...
- (id)initWithTable:(NSArray *)_arrTable{
    ...
    widthTable = ((ArticleTable *)_arrTable[0]).bounds.size.width;
    heightTable = ((ArticleTable *)_arrTable[0]).bounds.size.height;
...
//    NSArray *arrStrBackground = [dictBackground allValues];
    ...
    for(int i =0 ; i < [_arrTable count];i++){
        //自クラスのarrTableに格納
        [arrTable addObject:_arrTable[i]];
        
        //画像に対応(辞書dictBackgroundと配列arrStrBackground)
        switch(((ArticleTable *)_arrTable[i]).tableType){
            case TableTypeArts:{
                [dictBackground
                 setObject:@"light.png"
...
//
            ...
            ((ArticleTable *)arrTable[0]).center = CGPointMake(WIDTHDEVICE/2, HEIGHTDEVICE/2);
            for(int i = 1;i < [arrTable count];i++){
                ((ArticleTable *)arrTable[i]).frame =
                CGRectMake(WIDTHDEVICE, 0,
                           ((ArticleTable *)arrTable[i]).bounds.size.width,
                           ((ArticleTable *)arrTable[i]).bounds.size.height);
            }
            
            for(int i = 0;i < [arrTable count];i++){
                [self addSubview:((ArticleTable *)arrTable[i])];
                NSLog(@"arrtable%d is %@", i, arrTable[i]);
            }
...
-(void)animateTableToCorrectLocation{
    ...
    [UIView
     ...
     animations:^{
         ...
         //center
         ((ArticleTable *)arrTable[noStatus]).center =
...
//                     HEIGHTDEVICE/2);
         ...
         for(int i = 0;i < noStatus;i++){
             ((ArticleTable *)arrTable[i]).center =
             CGPointMake(noStatus * WIDTHDEVICE - widthTable/2,
                         heightTable/2);
         }
//         NSLog(@"左側center=%f", ((ArticleTable *)arrTable[noStatus-1]).center.x - self.frame.origin.x);
...
-(void)onFlickedFrame:(UIPanGestureRecognizer *)gr{
    ...
    //中央のテーブル
    ((ArticleTable *)arrTable[noStatus]).center =
    CGPointMake(((ArticleTable *)arrTable[noStatus]).center.x - movingPoint.x/2,
                ((ArticleTable *)arrTable[noStatus]).center.y);
    
    
    //左隣を画面上に表示
    if(noStatus != 0){
        ((ArticleTable *)arrTable[noStatus-1]).center =
        CGPointMake(((ArticleTable *)arrTable[noStatus-1]).center.x - movingPoint.x/2,
                    ((ArticleTable *)arrTable[noStatus-1]).center.y);
    }
